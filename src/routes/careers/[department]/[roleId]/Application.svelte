<script lang="ts">
	...
	let cvFile;
	let cvFileName;
	let coverLetterFile: any;
	let coverLetterFileName: any;

	export let form: ActionData;

	function handleCVFileChange(event: Event & { currentTarget: EventTarget & HTMLInputElement }) {
		const fileInput = event.target as HTMLInputElement;
		cvFile = fileInput.files ? fileInput.files[0] : null;
	...
	function handleCoverLetterFileChange(
		...
		const fileInput = event.target as HTMLInputElement;
		coverLetterFile = fileInput.files ? fileInput.files[0] : null;
		coverLetterFileName = coverLetterFile ? coverLetterFile.name : '';
	}
</script>

<form method="POST" class="max-w-[460px] grid gap-[25px] my-7" use:enhance>
	{JSON.stringify(form)}
	...
	<div class="form-group relative file-upload-field">
		<input type="file" name="cv" on:change={handleCVFileChange} />
		<input
			type="text"
			class="input-field text-midnight-blue dark:bg-black dark:text-white"
			required
	...
	<div class="form-group relative file-upload-field">
		<input type="file" name="cover_letter" on:change={handleCoverLetterFileChange} />
		<input
			type="text"
			bind:value={coverLetterFileName}
			class="input-field text-midnight-blue dark:bg-black dark:text-white"
			required
		/>
	...
	<div class="form-group textarea-wrapper">
		<div class="message-label mb-[10px]">
			Why have you chosen to apply to CWG? <span class="text-red">*</span>
		</div>
		<textarea
			rows="4"
	...
	<div class="form-group select-wrapper">
		<div class="message-label mb-[10px]">
			Would you require visa sponsorship to work in the country in which this position is
			based?
			<span class="text-red">*</span>
		</div>
		<select name="requires_sponsorship" required>
	...
	<div class="form-group select-wrapper">
		<div class="message-label mb-[10px]">
			...
			<div class="text-[12px]">
				I consent to the collection, use and disclosure of personal data relating to my
				gender by CWG in order for CWG to measure its efforts to create a diverse workplace.
				I understand that it is entirely voluntary for me to provide this information, and
				declining to provide CWG with consent to collect this information will not subject
				me to adverse treatment. I further understand that all individuals seeking
				employment or secondment at CWG are considered based on individual merit, without
				regard to such information; that this data will not be shared with any personnel who
				can influence the hiring process; and that I am being given the opportunity to
				provide such information in order to help CWG measure CWG's own diversity and
				inclusion efforts
			</div>
		</div>
	...
	<div class="form-group checkbox-group flex items-start gap-3 mt-[30px]">
		<input name="consent_given" type="checkbox" required />
