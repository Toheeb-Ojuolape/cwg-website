<script lang="ts">
	...
	export let esteemedClients: PartnersPage['esteemed_clients']['data'];
	...
	const commonClasses =
		'logos-wrapper flex flex-wrap gap-[40px] max-w-[1380px] m-auto justify-center px-5';
...
<div class={showMore ? `${commonClasses} show-more` : `${commonClasses}`}>
	{#each esteemedClients as { attributes: { logo, name, logo_dark } }}
		<img
			src={CMS_URL +
				($theme === 'light'
...
<style>
	...
	.logos-wrapper.show-more {
		max-height: 300px; /* Adjust this value based on the height you want to show initially */
		overflow: hidden;
		position: relative;
	}
